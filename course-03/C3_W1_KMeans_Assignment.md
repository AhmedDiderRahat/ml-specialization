...
def find_closest_centroids(X, centroids):
    ...
    # c is matrix of data points x number of K
    c = np.zeros((X.shape[0], K))

    for i in range(len(X)):
        for j in range(K):
            c[i][j] = np.sum((X[i] - centroids[j])**2)
...
def compute_centroids(X, idx, K):
    ...
    for k in range(K):
        # get the indecies which has the value k
        indices = np.where(idx == k)[0]
        
        # Select rows with indices 1 and 3
        selected_rows = X[indices, :]
        
        # Compute the column-wise average for the selected rows
        column_wise_average = np.average(selected_rows, axis=0)
        
        # assign the average
        centroids[k] = column_wise_average
...
```
    ...
    <matplotlib.image.AxesImage at 0x7e243471ab90>
